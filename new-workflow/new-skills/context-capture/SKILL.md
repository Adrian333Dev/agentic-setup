...
**Passively** — always on, no invocation needed. The agent checks continuously during any working phase and writes when something surfaces.

**On user request** — the user can say "save this," "add to backlog," "checkpoint," or invoke `/context-capture` explicitly. When invoked directly, the agent either routes what the user specified or, if no specific content was given, writes a session checkpoint.
