...
`brainstorm.md` lives in `docs/work/topics/<slug>/brainstorm.md`. Create the topic folder if it doesn't exist. The slug is a short kebab-case name for what's being designed (e.g., `auth-system`, `billing-pipeline`). Milestone folders come later — after spec is complete and scope is understood.
...
- **Never print the tree and walk a branch in the same message.** Phase 1 (orient + print tree) is one message. Phase 2 starts in the next message with exactly one branch question + recommendation.
